...
with tab_app_cb:
    ...
    with col_cb_acc_grave:
        ...
        acc_types = st.multiselect(label="Type d'accident", options=refs.dic_grav_sht.values(), label_visibility="collapsed")
        if 'acc_types' not in st.session_state.keys(): st.session_state['acc_type'] = acc_types

    with col_pred_type:
        pred_types = st.multiselect(label="Prédiction", options=refs.dic_pred_type.values(), label_visibility="collapsed")
        if 'pred_types' not in st.session_state.keys(): st.session_state['pred_types'] = pred_types
